...
class Post extends Model
{
    ...
    {
        return $this->db->table($this->table)->where($data)->orderBy("id_post", "DESC")->get(1)->getRowArray();
    }
    ...
    {
        return $this->db->table($this->table)->update($data, ["id_post" => $id]);
    }
    ...
    {
        return $this->db->table($this->table)->delete(["id_post" => $id]);
    }
    ...
    function up()
    {
        return $this->db->query("CREATE TABLE `post` (`id_post` int(11) NOT NULL auto_increment,`id_user` int(11) NOT NULL ,`status` text NOT NULL ,`entry_date` datetime NULL ,`delete_date` datetime NULL ,`update_date` datetime NULL ,    PRIMARY KEY (`id_post`)
